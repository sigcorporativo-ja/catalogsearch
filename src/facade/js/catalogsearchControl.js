...
@namespace("M.control")
export class CatalogSearchControl extends M.Control {
 ...
 */
  ...
  constructor(geoNetworkUrl) {
    ...
    if (M.utils.isUndefined(M.impl.control.CatalogSearchControl)) {
      M.exception('La implementación usada no puede crear controles CatalogSearchControl');
    }
    // 2. implementation of this control
    let impl = new M.impl.control.CatalogSearchControl();
  ...
  createView(map) {
    ...
    let promise = new Promise((success, fail) => {
      M.template.compile(CatalogSearchControl.TEMPLATE, {
        'jsonp': true
      }).then((html) => {
        this.addEvents(html);
        success(html);
  ...
  addEvents(html) {
    ...
    // search buntton
    let btnSearch = this.element_.getElementsByTagName('button')["m-catalogsearch-search-btn"];
    btnSearch.addEventListener("click", (evt) => this.searchClick_(evt));

    // clear buntton
    let btnClean = this.element_.getElementsByTagName('button')["m-catalogsearch-clear-btn"];
        ...
        xml2js.parseString(response.text, { explicitArray: false }, (err, parse) => {
          ...
          if (results.total > 0) {
            ...
            metadatos.forEach((metadata) => {
              ...
              promises.push(this.search_uuid(uuid, searchTime).then((metadata) => {
  ...
  search_uuid(uuid, searchTime) {
    return new Promise((success, fail) => {
      let searchUrl = M.utils.addParameters(this.geoNetworkUrl_ + '/xml.metadata.get', {
        'uuid': uuid
      });
        ...
        if (searchTime === this.searchTime_) {
          xml2js.parseString(response.text, { explicitArray: false }, (err, result) => {
            ...
            if (metadata.type == 'dataset' || metadata.type == 'series') {
              let metadataWMSUrl = this.checkNestedJSON_(result, 'gmd:MD_Metadata', 'gmd:distributionInfo', 'gmd:MD_Distribution', 'gmd:distributor',
              ...
              if (Array.isArray(metadataWMSUrl)) {
                for (let j = 0; j < (metadataWMSUrl.length - 1); j++) {
                  let data = this.checkNestedJSON_(metadataWMSUrl[j], 'gmd:CI_OnlineResource', 'gmd:protocol', 'gco:CharacterString');
                ...
                let data = this.checkNestedJSON_(metadataWMSUrl, 'gmd:CI_OnlineResource', 'gmd:protocol', 'gco:CharacterString');
                if (!M.utils.isNullOrEmpty(data) && data.trim() == 'OGC:WMS-1.3.0-http-get-capabilities') {
              ...
              let metadataWMSUrl = this.checkNestedJSON_(result, 'gmd:MD_Metadata', 'gmd:identificationInfo', 'srv:SV_ServiceIdentification',
                'srv:containsOperations');
              if (Array.isArray(metadataWMSUrl)) {
                for (let j = 0; j < (metadataWMSUrl.length - 1); j++) {
                  let service = this.checkNestedJSON_(metadataWMSUrl[j], 'srv:SV_OperationMetadata', 'srv:operationName', 'gco:CharacterString');
                ...
                let service = this.checkNestedJSON_(metadataWMSUrl, 'srv:SV_OperationMetadata', 'srv:operationName', 'gco:CharacterString');
                if (!M.utils.isNullOrEmpty(service) && service.trim() == 'GetCapabilities') {
  ...
  search_layers(evt) {
    ...
    let layersContainer = evt.target.parentNode.parentNode.querySelector('div.layers-list');
    ...
    if (M.utils.isNullOrEmpty(layersContainer.innerHTML)) {
      this.getImpl().getLayersFromWMSCapabilities(url).then((capabilities) => {
        this.element_.classList.remove(CatalogSearchControl.SEARCHING_CLASS);
        if (!M.utils.isNullOrEmpty(capabilities) && !M.utils.isNullOrEmpty(capabilities.Capability.Layer)) {
  ...
  showLayers_(url, layers, layersContainer) {
    ...
    M.template.compile(CatalogSearchControl.LAYERS_TEMPLATE, {
      'jsonp': true,
  ...
  add_layers(evt) {
    evt = (evt || window.event);
    let url = evt.target.dataset.url;
    let listLayersElement = evt.target.parentNode.querySelectorAll('input[type=checkbox]');
    // loop over them all
  ...
  showFullMetadata(evt) {
    ...
    let metadataContainer = evt.target.parentNode.parentNode.querySelector('div.metadata-template');
    ...
    if (M.utils.isNullOrEmpty(metadataContainer.innerHTML)) {
      M.remote.get(url).then((response) => {
        ...
        // code for Chrome, Firefox, Opera, etc.
          ...
          let resultDocument = xsltProcessor.transformToFragment(response.xml, document);
          metadataContainer.innerHTML = '';
   ...
   */
  showResults_() {
    M.template.compile(CatalogSearchControl.RESULTS_TEMPLATE, {
      'jsonp': true,
      'vars': this.results_
    }).then((html) => {
  ...
  appendResults_(services) {
    ...
    M.template.compile(CatalogSearchControl.RESULTS_TEMPLATE, {
      ...
      let spanNumFound = this.resultsContainer_.querySelector("span#m-catalogsearch-page-found");
   ...
   */
  checkScrollSearch_() {
    if ((this.results_.services.length == this.results_.total) && (!M.utils.isNullOrEmpty(this.resultsScrollContainer_))) {
      this.resultsScrollContainer_.removeEventListener("scroll", this.boundResultsScroll_);
    }
  }
